...
◎ **Marcel法**　打者誤差 .055（OPS）／ 投手誤差 0.62（ERA）
△ AI（LightGBM）　打者誤差 .077（OPS）／ 投手誤差 0.95（ERA）

### 2025年検証

◎ **Marcel法**　打者誤差 .048（OPS）／ 投手誤差 0.63（ERA）
△ AI（XGBoost）　打者誤差 .062（OPS）／ 投手誤差 0.92（ERA）
...
2024年の不振に引っ張られ、低めに予測してしまいました。しかし2025年の筒香選手は見事に復活。こういう「前年までのトレンドを大きく上回る急激な成績変化」は、Marcel法が捉えにくい領域と考えられます。
...
1位　近藤健介（249）― リーグ平均の2.5倍の得点力
2位　オースティン（248）― 同上
3位　サンタナ（220）― リーグ平均の2.2倍
...
「牧秀悟の翌年予測は？」という問いに、こんな形で答えるシステムも作りました。
...
GitHubのコードをローカルで動かすと試せます（Pythonが必要です）。

**起動手順**

```
git clone https://github.com/yasumorishima/npb-prediction
cd npb-prediction
